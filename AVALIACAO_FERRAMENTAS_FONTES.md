...
## Auditoria dos repositórios reais

Em 7 de agosto de 2026, o repositório oficial `n8n-io/n8n` foi clonado em `integrations/n8n` no commit `62e6862f`. O pacote CLI auditado declara a versão `2.34.0`, usa pnpm e possui o comando de inicialização oficial `node scripts/os-normalize.mjs --dir packages/cli/bin n8n`. A licença encontrada é a Sustainable Use License. Ela permite uso interno de negócio, mas restringe oferecer o software como produto ou serviço concorrente. Por isso, o PerfilPro preparou um serviço n8n separado para automações internas da agência; não está oferecendo n8n como white-label.

O repositório oficial `baptisteArno/typebot.io` também foi clonado em `integrations/typebot` no commit `248433d`. A licença encontrada é `FSL-1.1-Apache-2.0`. A cláusula de Competing Use exclui disponibilizar o software em produto ou serviço comercial que substitua o Typebot ou ofereça funcionalidade substancialmente semelhante. A licença inclui uso interno e serviços profissionais prestados a um licenciado, mas qualquer uso para clientes deve ser validado caso a caso. Por segurança, o Typebot não será incorporado ao SaaS principal nem oferecido como white-label; o PerfilPro continuará usando seus próprios formulários e webhooks para captação até haver uma decisão jurídica/comercial específica.
